...
object UniqueItemRegistry {
    ...
    fun registerToolItem(factory: (Item.Properties) -> Item, settings: Item.Properties, name: String): Item {
        val item = RegistryUtil.registerItem(factory, settings, name)
        _entries[name] = item
        return item
    }

    fun registerMiscItem(factory: (Item.Properties) -> Item, settings: Item.Properties, name: String) = registerToolItem(factory, settings, name)
}